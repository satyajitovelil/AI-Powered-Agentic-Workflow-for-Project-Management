...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions):# TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": eval_prompt}],# TODO: 5 - Define the message structure sent to the LLM for evaluation (use temperature=0)
                temperature=0
            )
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    messages=[{"role": "user", "content": instruction_prompt}],# TODO: 6 - Define the message structure sent to the LLM to generate correction instructions (use temperature=0)
                    temperature=0
                )
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        steps = [line.strip() for line in response_text.splitlines() if line.strip()]
